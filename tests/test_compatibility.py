...
def test_every_plugin_resolves_on_the_schema_capable_app_line(registry: dict):
    """Nothing published may be unreachable by the newest supported line."""
    problems = []
    for plugin_id, entries in _entries_by_id(registry).items():
        if _latest_compatible(entries, SCHEMA_V3_APP_VERSION) is None:
            problems.append(
                f"{plugin_id} has no version compatible with {SCHEMA_V3_APP_VERSION}"
            )
    assert not problems, "\n".join(problems)


def test_long_lived_plugins_keep_a_version_for_the_oldest_supported_line(registry: dict):
    """Plugins that already shipped for the oldest served line keep one.

    Plugins introduced later (the community cluster providers declare
    ``>=1.5.8``) legitimately have no version for an older line; that is a
    publication date, not a compatibility regression.
    """
    by_id = _entries_by_id(registry)
    for plugin_id in ("org.hpcclient.truba", "org.hpcclient.fluent"):
        assert _latest_compatible(by_id[plugin_id], CURRENT_APP_VERSION) is not None, (
            f"{plugin_id} lost its {CURRENT_APP_VERSION} compatible version"
        )
