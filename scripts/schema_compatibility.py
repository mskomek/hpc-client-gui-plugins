...
#
# Package bytes are immutable once published (see ``published_lock.py``), but
# a compatibility *decision* can turn out wrong after publication. A registry
# entry may therefore carry:
#
#   "compatibility_override": {
#     "requires_app": ">=1.5.9",
#     "reason": "...", "recorded": "YYYY-MM-DD"
#   }
#
# Precedence:
#   effective_requires_app = override.requires_app if present
#                            else the immutable manifest requires_app
#
# An override may only *narrow* compatibility - raise the floor, never lower
# it. Widening would let the registry advertise an install the application
# must reject. The override affects discovery, offerability, and resolution
# only; the installer's unsupported-schema rejection, payload validation, and
# hash integrity checks remain the final authority and are never consulted
# through this field.


def effective_requires_app(entry: dict) -> str:
    """The compatibility range the registry actually advertises for ``entry``."""
    override = entry.get("compatibility_override")
    if isinstance(override, dict):
        value = override.get("requires_app")
        if isinstance(value, str) and value:
            return value
    return str(entry["requires_app"])


def override_error(entry: dict) -> str | None:
    """Reject an override that widens compatibility or is malformed."""
    override = entry.get("compatibility_override")
    if override is None:
        return None
    if not isinstance(override, dict):
        return "compatibility_override must be an object"
    value = override.get("requires_app")
    if not isinstance(value, str) or not value:
        return "compatibility_override.requires_app must be a non-empty string"
    if not str(override.get("reason", "")).strip():
        return "compatibility_override.reason is required"
    declared = _minimum_admitted(str(entry["requires_app"]))
    overridden = _minimum_admitted(value)
    if overridden is None:
        return f"compatibility_override.requires_app {value!r} is not a valid range"
    if declared is not None and overridden < declared:
        return (
            f"compatibility_override.requires_app {value!r} would admit releases "
            f"older than the published requires_app {entry['requires_app']!r}; "
            "an override may only narrow compatibility"
        )
    return None
