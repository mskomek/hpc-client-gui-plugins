...
defaults for HPC Client GUI with dynamic output channel definitions. It does
not imply MareNostrum 5 behavior and is not an official TÜBİTAK ULAKBİM/TRUBA
client.
...
This version declares two automatic output channels (`stdout` and `stderr`)
using Slurm runtime resolution. The actual file paths come from the scheduler
at runtime, not from hardcoded filenames.
